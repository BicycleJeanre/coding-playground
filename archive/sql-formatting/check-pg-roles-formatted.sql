...
,       CASE 
        ...
        END AS privilege
,       CASE 
            WHEN pg_catalog.has_database_privilege(rolname, datname, 'CONNECT') 
                AND pg_catalog.has_database_privilege('public', datname, 'CONNECT') THEN 'PUBLIC'
            ELSE NULL
    ...
    CROSS JOIN 
        pg_catalog.pg_roles
    WHERE 
        (pg_catalog.has_database_privilege(pg_roles.rolname, pg_database.datname, 'CONNECT')
...
,           rolname AS inherited_from
        FROM 
            pg_catalog.pg_auth_members
        JOIN 
            pg_catalog.pg_roles AS inherited_roles ON pg_auth_members.roleid = inherited_roles.oid
    ) AS inherited_from_roles ON pg_roles.oid = inherited_from_roles.member_oid
    WHERE 
        (rolcanlogin OR rolsuper OR rolcreaterole OR rolcreatedb OR rolreplication OR rolinherit)
...
,       grantor AS inherited_from
    ...
    LEFT JOIN 
        pg_catalog.pg_class cls ON ns.oid = cls.relnamespace
    LEFT JOIN 
        information_schema.role_table_grants grants ON grants.table_schema = ns.nspname AND grants.table_name = cls.relname
) privileges
WHERE 
    role_name NOT LIKE 'pg_%'
